...
class QMemoryManagerPixmap {
    ...
    bool allocated;                 // false if set via setRawData
    uchar* data;
    int linestep;                   // only used when setRawData called
};

class QMemoryManagerMoto;
...
public:
    ...
    uchar *pixmapData(PixmapID id) { return (*(pixmap_map.find(id))).data; }

    int calcLineStep(bool id, int width, int depth);
    int pixmapLinestep(PixmapID id, int width, int depth);
    ...
    //----
    // added so we can share pixmaps between processes
    void setRawData(PixmapID id, uchar* data, int linestep, bool alloc = false);
    //----
...
private:
    ...
    friend class QMemoryManagerMoto;
    QMemoryManagerMoto *d;

public:
    QMemoryManagerMoto *moto() const { return d; }
};
