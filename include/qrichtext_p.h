/****************************************************************************
** $Id: qt/src/kernel/qrichtext_p.h   2.3.8   edited 2004-08-05 $
**
...
class QTextRichString
{
	...
	Item()
#ifndef QT_NO_WTLE
	: width(-1),newline(0),breakable(0),right2left(0),format(0), layout(NULL), tstext(NULL)
#else
...
#ifndef QT_NO_WTLE
	Item(const Item &i)
	: base(i.base),width(i.width),newline(i.newline),breakable(i.breakable),right2left(i.right2left),format(i.format),c(i.c),layout(NULL),tstext(NULL)
	{
	};
	Item &operator=(const Item &i)
	{
	    if (layout) {
		TsLayout_delete(layout);
		layout = NULL;
	    }
	    if (tstext) {
		TsText_delete(tstext);
		tstext = NULL;
	    }
	    base= i.base;
	    width = i.width;
...
public:
    ...
    void insert( int index, const QString& c, const QTextCharFormat& fmt, int breakable=FALSE, int right2left=FALSE );
    void append( const QString& c,const  QTextCharFormat& fmt, int breakable=FALSE, int right2left=FALSE );
    void clear();
    ...
    int overallR2L;
    void dirty();
    void genTsLayout(int index);
    int movePosition(int index, int offset, TsCaretPosition type);
    int width(int index, int b = 0, int len = -1);
#endif
...
//     void updateParagraph( QPainter* );
    ...
    void gotoLineStart( QPainter* p = 0, int drawing=FALSE );
    ...
    void drawLabel( QPainter* p, QTextParagraph* par, int x, int y, int w, int h, int ox, int oy,
		   QRegion& backgroundRegion,
		   const QColorGroup& cg, const QTextOptions& to );
    void gotoNextItem( QPainter* p = 0 , int drawing=FALSE);
...
inline void QTextRichString::append( const QString& c,const  QTextCharFormat& fmt, int breakable, int right2left )
{
	insert( length(), c, fmt, breakable, right2left);
}
