...
/****************************************************************************
** $Id: qt/src/kernel/qwsevent_qws.h   2.3.6   edited 2003-02-13 $
**
...
struct QWSEvent : QWSProtocolItem {
    ...
    // ZCopChannel hardcopied all this enum type. 
    // Any changes should applies to Zcopchannel
    // No need to enclose this definition in macros. ZCopchannel don't know these QT macro.
    enum Type { 
	NoEvent,
	Connected,
	Mouse, Focus, Key,
...
struct QWSKeyEvent: QWSEvent {
    QWSKeyEvent()
        : QWSEvent( QWSEvent::Key, sizeof( simpleData ), (char*)&simpleData ) {}
    struct SimpleData {
        int window;
        ushort unicode;
        ushort keycode;
...
#ifndef QT_NO_QWS_MMI_TEST
struct QWSCaptureEvent: QWSEvent {
    QWSCaptureEvent()
	: QWSEvent( QWSEvent::Capture, sizeof( simpleData ),
	(char*)&simpleData ) {}
    struct SimpleData {
	int window;
    } simpleData;
...
#ifdef QT_QWS_MULTI_DISPLAY
struct QWSScreenSwitchEvent : QWSEvent {
    QWSScreenSwitchEvent()
	: QWSEvent( ScreenSwitch, sizeof( simpleData ), (char *)&simpleData ) { }
