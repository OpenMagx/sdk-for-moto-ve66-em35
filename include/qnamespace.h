...
/****************************************************************************
** $Id: qt/src/kernel/qnamespace.h   2.3.6   edited 2002-07-16 $
**
...
public:
    ...
    // support multiple displays in one application, no need to enclosed in macro QT_QWS_MULTI_DISPLAY 
    enum DisplayID {
	ActiveDisplay    = -2,    // current active display in the system, most function use it as default value
	AllDisplay       = -1,    // all displays in the system, used by functions that do not distinguish displays
	MainDisplay      = 0,     // the main display, also called display0, must be 0 when converted to int
	ExternalDisplay  = 1,     // the external display, or CLI display, also called display1, must be 1 when converted to int
	//ThirdDisplay   = 2,     //Maybe
    };
	...
	WStyle_UseDialogWallpaper=0x20000000, // use dialog wallpaper
	WStyle_BlendWallpaper   = 0x40000000, // force blending
	WStyle_NoBlendWallpaper = 0x80000000, // force not blending
	WStyle_InheritBlendWallpaper = 0x0
    };
	...
	Key_Hangup = 0xffe9,
#endif
	Key_Omg_Touch       = 0x4010,
	Key_Omg_Pageup      = 0x4011,
	Key_Omg_Pagedown    = 0x4012,
	Key_Omg_Scroll      = 0x4013,
	Key_Omg_Stop        = 0x4014,
	Key_Omg_Resume      = 0x4015,
	Key_Omg_Rate        = 0x4016,
