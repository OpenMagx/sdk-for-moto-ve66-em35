...
/****************************************************************************
** $Id: qt/src/kernel/qpainter.h   2.3.8   edited 2004-08-05 $
**
...
//    void	setUnit( PaintUnit );		// NOT IMPLEMENTED!!!
  ...
  // Graphics drawing functions
    ...
    // 
    // Methods added by Motorola to support erasing with an arbitrary
    // color.  See notes in the inline implementations below
    //
    
    void    eraseRect( int x, int y, int w, int h, const QBrush & );
    void    eraseRect( const QRect &, const QBrush & );
...
// add a switch   to alter Number between English and Arabic  
    bool    alter_number; 
    void    setAlterNumber(bool value) { cfont.setAlterNumber(value);};
// add a switch   to alter Number between English and Arabic
...
private:	// Disabled copy constructor and operator=
    ...
    friend class QPainterMoto;
    QPainterMoto *d;
public:
    QPainterMoto *moto() const { return d; }
};
...
// 
// These functions are here to enable blending of wallpaper and the
// background color.  They are always compiled.  
//
// If QT_BLEND_WALLPAPER is not defined, eraseRect() will end up 
// calling fillRect in qpainter_qws.cpp.
//

inline void QPainter::eraseRect( int x, int y, int w, int h )
{
    eraseRect(x, y, w, h, backgroundColor() );
}

inline void QPainter::eraseRect( const QRect &r )
{
    eraseRect( r.x(), r.y(), r.width(), r.height(), backgroundColor() );
}

//
// Note: the following method was added so that we can erase to an
// arbitrary color.
//
// In standard Qt, you call fillRect(...) with an arbitrary color to
// create a rectangle of a given color.  However, because we've changed
// eraseRect() to blend with the wallpaper, but not fillRect(), there's
// no way for an application to blend the wallpaper with an arbitrary color.
//
// This method allows you to do that.
//

inline void QPainter::eraseRect( const QRect &r, const QBrush& c )
{
    eraseRect( r.x(), r.y(), r.width(), r.height(), c );
}
...
// 
// standard Qt implementation -- removed to enable blending of wallpaper
// and the background color.  Note: if QT_BLEND_WALLPAPER is not defined,
// qpainter_qws.cpp will cause eraseRect(int,int,int,int,const QBrush&)
// to call fillRect().  So the default Qt behavior will be maintained.
//
