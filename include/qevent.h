...
/****************************************************************************
** $Id: qt/src/kernel/qevent.h   2.3.6   edited 2002-07-08 $
**
...
//#ifdef QT_QWS_MULTI_DISPLAY
	Migrate = 110,                          // migrate widget
	DisplaySwitch = 111,                     // active screen display switches
//#endif
...
public:
    QKeyEvent( Type type, int key, int ascii, int state, const QString& text=QString::null, 
            bool autorep=FALSE, ushort count=1, int step=0 )
	: QEvent(type), txt(text), k((ushort)key), s((ushort)state),
	    a((uchar)ascii), accpt(TRUE), autor(autorep), c(count), steps(step) {}
...
//#ifdef QT_QWS_MULTI_DISPLAY
class Q_EXPORT QDisplaySwitchEvent : public QEvent
{
public:
    QDisplaySwitchEvent( DisplayID s ) : QEvent(DisplaySwitch), actived(s) {}
    DisplayID activeDisplay() const { return actived; }
    
private:
    DisplayID actived;
};

class Q_EXPORT QMigrateEvent : public QEvent
{
public:
    QMigrateEvent( DisplayID old_dis, DisplayID dst_dis ) 
	: QEvent(Migrate), oldd(old_dis), d(dst_dis) {}
    DisplayID oldDisplay() const { return oldd; }
    DisplayID display() const { return d; }
