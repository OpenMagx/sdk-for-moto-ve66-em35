...
/****************************************************************************
** $Id: qt/src/kernel/qapplication.h   2.3.6   edited 2001-10-05 $
**
...
public:
    ...
    int		    argc()	const;
    char	  **argv()	const;
    bool      isquiting(){return quit_now;}
    Type type() const;
    ...
    static QWidgetList *allWidgets(DisplayID id=AllDisplay);
    static QWidgetList *topLevelWidgets(DisplayID id=AllDisplay);
    static QWidget     *desktop(DisplayID id=ActiveDisplay);
    ...
    static QWidget  *widgetAt( int x, int y, bool child=FALSE, DisplayID id=ActiveDisplay ); 
    static QWidget  *widgetAt( const QPoint &, bool child=FALSE, DisplayID id=ActiveDisplay );
    ...
    static void      setOmegaWheelScrollLines( int );
    static int       omegaWheelScrollLines();
    ...
    static Qt::DisplayID     activeDisplayId();
    ...
    //add by to support performance of Multi-language layout engine 2006/02/23
    void setOpenLayoutEngine(bool bOpen);
    bool openLayoutEngine();
    //end 

    // Added to get/set top application name in container, such as FL container.
    const char* getUiFlowName();
    void setUiFlowName(const QString new_name);
...
#if defined(_WS_QWS_)  // keep this signal even without macro QT_QWS_MULTI_DISPLAY defined
    void	    displaySwitched(Qt::DisplayID);
#endif
...
#ifdef QT_QWS_MULTI_DISPLAY
    bool translateScreenSwitchEvent( const QWSScreenSwitchEvent * );
#endif

#ifndef QT_NO_QWS_MMI_TEST
public:
    void reportWindow( int wid );
#endif
};
...
inline QWidget *QApplication::widgetAt( const QPoint &p, bool child, DisplayID display )
{
    return widgetAt( p.x(), p.y(), child, display );
}
