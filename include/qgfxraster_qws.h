...
/****************************************************************************
** $Id: qt/src/kernel/qgfxraster_qws.h   2.3.8   edited 2004-08-05 $
**
...
public:
    ...
    virtual PixelType pixeltype() {return pixtype;};
...
protected:
    ...
    unsigned int get_value_18( int sdepth, unsigned char **srcdata, 
                           bool reverse=FALSE);
#endif

#ifndef QT_NO_QWS_DEPTH_24_ALPHA
    unsigned int get_value_24alpha( int sdepth, unsigned char **srcdata,
                           bool reverse=FALSE);
#endif
protected:
...
protected:
    friend class QGfxRasterMoto;
    QGfxRasterMoto *d;

public:
    QGfxRasterMoto *moto() const { return d; }
...
public:
    ...
    // for blending background with wallpaper
    virtual void eraseRect( int,int,int,int,int,const QPixmap* );
#endif
