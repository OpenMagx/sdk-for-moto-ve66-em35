...
/**********************************************************************
** $Id: qt/src/widgets/qmultilineedit.h   2.3.8   edited 2004-08-05 $
**
...
private:
	...
	QMultiLineEditRow( QString string, int width, bool nl = TRUE, bool old = FALSE, eParaAlignment dir = DIR_UNKNOWN )
...
public:
    ...
    int  isLTRBidiClass(TsBidiCharClass);
    int  isRTLBidiClass(TsBidiCharClass);
    int  isNeutralBidiClass(TsBidiCharClass); 
    int  isNumericBidiClass(TsBidiCharClass);
    int  isLTRM(TsBidiCharClass txtClass);
    int  isSpaceNeutral(TsBidiCharClass txtClass);
...
#ifndef QT_NO_WTLE
    int  pixelPosToIMPos( const QPoint& );
#else
