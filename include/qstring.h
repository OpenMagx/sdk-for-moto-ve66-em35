...
/****************************************************************************
** $Id: qt/src/tools/qstring.h   2.3.6   edited 2001-04-03 $
**
...
#error "fill() Should return *this, or QChar constructor should take count=1"
   ...
   QString& append(char const* s, int len);
   QString arg(long long a, int fieldwidth = 0, int base = 10) const;
   QString arg(unsigned long long a, int fieldwidth = 0, int base = 10) const;
   QString fromAscii(char const* s, int len);
   QString& prepend(char const* s1);
   QString    &setNum( long long, int base=10 );
   QString    &setNum( unsigned long long, int base=10 );
   static QString number( long long, int base=10);
   static QString number( unsigned long long, int base=10);
   QString    &operator+=( const QCString& cstr );         // MOTO_CHANGES
   QString    &operator+=( const QByteArray& byteArray );  // MOTO_CHANGES
   QString    &operator+=( const char* cstr );             // MOTO_CHANGES
...
//Added for AMTS
#ifndef QT_NO_QWS_MMI_TEST
    inline const char* getResid() const {return d->resid;}
    inline void setResid(const char* r){
        if(d->resid)	
        {
            delete[] d->resid;
            d->resid=0;
        ...
        {
            int len=strlen(r)+1;
            d->resid=new char[len]; 
            memcpy((void*)d->resid, r, len); 
        }
        return;
    }
#endif
