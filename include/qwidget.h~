...
/****************************************************************************
** $Id: qt/src/kernel/qwidget.h   2.3.6   edited 2003-05-22 $
**
...
class Q_EXPORT QWidget : public QObject, public QPaintDevice
{
    ...
    // To remove the RESET unsetBackground in order to remove the complilation warning
...
public:
    QWidget( QWidget *parent=0, const char *name=0, WFlags f=0, DisplayID displayId=MainDisplay );
    ~QWidget();

    WId		 winId() const;
    void	 setName( const char *name );
    // Morphing Feature
    void    setMorphMode( const int morphMode ); //Set morph mode for current TLW
    ...
    void         setWidgetType(bool b) {isWidget = b;}

// Added - begin.
// To focus in widget priority order.
// WARNING! You SHOULD NOT call this unless you know what you are doing.
#ifdef _WS_QWS_
    int regionPriority() const { return( isTopLevel() ? region_priority : 0); }
    void setRegionPriority( int level );
    
    DisplayID displayId() const { return displayid; }
    
    virtual void migrate( DisplayID display );
#endif
// Added - end.
...
public:
    ...
    // for supporting lcd overlay
    bool isOverlayBackgroundEnabled() const { return overlayBgEnabed; }
    void enableOverlayBackground(bool enable, bool isSetChildren = true);
    
protected:
    bool overlayBgEnabed;
...
#if defined(_WS_QWS_) //&& defined(QT_QWS_MULTI_DISPLAY)
...
//#ifndef QT_NO_QWS_FADEBACKGROUND //leave interface  
    void        setFadeoverMode(FadeoverMode m=HierarchicFade); 
    void        setSkipFadeover(bool sk=true);
//#endif
/*
    void setWinTransparency(Inherit | On | Off);
    xxxx wTransparency() const;
...
private:
 ...
 private:
    int     bellkeypad_counter;// Morphing: 0 - bell keypad off; other - on
#ifndef QT_NO_PALETTE
    QPalette	 pal;
#endif
    QFont	 fnt;
    ...
    void setDisplayIdLocally(DisplayID id);
...
// Added - begin.
// To focus in widget priority order.
    // To indicate the region priority, should be same with the flag in QWS.
    // The common value is 0. Refer to UTIL_PrioritizedWidget.h.
    int region_priority;
// Added - end.
    ...
    static QWidgetList	 *wList(DisplayID);
    static QWidgetList	 *tlwList(DisplayID);
...
public:
    // Morphing Feature
    bool isBellKeypadRequired() const;
    bool isAppSpecMode() const;
    void useAppSpecMode();

protected:
    void requireBellKeypad();

private:
    void increaseBellKeypadCounter();
    void decreaseBellKeypadCounter();

#ifndef QT_NO_QWS_MMI_TEST
public:
    QPTF* qptf() const { return d; }
private:
    QPTF* d;
#endif
...
struct QTLWExtra {
    ...
    int      morphMode;          // morph mode
    bool     is_appspec_mode;    // true - application specified mode; false - automatic mode prediction    
};
...
inline bool QWidget::isBellKeypadRequired() const
{
    return !( 0 == this->bellkeypad_counter );
}

inline bool QWidget::isAppSpecMode() const
{
    return this->topLevelWidget()->topData()->is_appspec_mode;
}

inline void QWidget::useAppSpecMode()
{
    this->topLevelWidget()->topData()->is_appspec_mode = true;
}

inline void QWidget::requireBellKeypad()
{
    if ( 0 == this->bellkeypad_counter )
        this->bellkeypad_counter = 1;
}
